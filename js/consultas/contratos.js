...
$.getJSON(SPARQL_ENDPOINT + '?query=' + encodeURIComponent(query) + '&format=application%2Fsparql-results%2Bjson&timeout=0')
    .success(function(data) {
		...
		for (var i = 0; i < data.results.bindings.length; i++) {
			...
			table += '<tr><td>' + '<a href="'+feature.uri.value+'">'+feature.Titulo.value+'</a>' + '</td><td>' + nombre.value + '</td><td>' + '<a href="adjudicadosCIF.html?cif='+cif.value+'">'+cif.value + '</a></td><td>' + '<a href="adjudicadosPorServicioGestor.html?servicio='+id.value+'">'+servicioGestor.value + '</a></td><td>' + Math.round(parseFloat(precio.value)).toLocaleString() + '</td><td>' + feature.fechaFormalizacion.value + '</td></tr>';   
